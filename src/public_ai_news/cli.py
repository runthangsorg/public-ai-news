...
from .sources import fetch_hacker_news
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    raw = (
        fetch_hacker_news(limit=min(args.max_items * 3, 50))
        if args.hacker_news
        else _load_input(args.input)
    )
    print(json.dumps(rank_items(raw, limit=args.max_items), indent=2, sort_keys=True))
    return 0
